...
export const requestPaymentType = async (): Promise<PaymentTypeModel[] | HttpErrorResponseModel> => {
  const controllerName = `PaymentType`;
  const endpoint: string = environment.api.generic.replace(':controller', controllerName);

  return EffectUtility.getToModel<PaymentTypeModel[]>(PaymentTypeModel, endpoint);
};
