...
export const requestCoverageHour = async (): Promise<CoverageHourModel[] | HttpErrorResponseModel> => {
  const controllerName = `CoverageHour`;
  const endpoint: string = environment.api.generic.replace(':controller', controllerName);

  return EffectUtility.getToModel<CoverageHourModel[]>(CoverageHourModel, endpoint);
};
