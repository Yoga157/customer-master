...
export const getPresalesView = async (): Promise<PresalesViewResult[] | HttpErrorResponseModel> => {
  const controllerName = 'Udc/GetByEntryKey/PresalesView';
  const endpoint: string = environment.api.generic.replace(':controller', controllerName);
  
  return EffectUtility.getToModel<PresalesViewResult[]>(PresalesViewResult, endpoint);
};
