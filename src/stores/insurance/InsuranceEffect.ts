...
export const requestInsurance = async (): Promise<InsuranceModel[] | HttpErrorResponseModel> => {
  const controllerName = `Insurance`;
  const endpoint: string = environment.api.generic.replace(':controller', controllerName);

  return EffectUtility.getToModel<InsuranceModel[]>(InsuranceModel, endpoint);
};
