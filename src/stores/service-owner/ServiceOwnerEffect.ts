...
export const requestServiceOwner = async (): Promise<ServiceOwnerModel[] | HttpErrorResponseModel> => {
  const controllerName = `ServiceOwner`;
  const endpoint: string = environment.api.generic.replace(':controller', controllerName);

  return EffectUtility.getToModel<ServiceOwnerModel[]>(ServiceOwnerModel, endpoint);
};
