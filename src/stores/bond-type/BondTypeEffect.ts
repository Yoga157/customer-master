...
export const requestBondType = async (): Promise<BondTypeModel[] | HttpErrorResponseModel> => {
  const controllerName = `BondType`;
  const endpoint: string = environment.api.generic.replace(':controller', controllerName);

  return EffectUtility.getToModel<BondTypeModel[]>(BondTypeModel, endpoint);
};
