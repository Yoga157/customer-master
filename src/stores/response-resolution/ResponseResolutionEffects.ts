...
export const requestResponseResolution = async (): Promise<ResponseResolutionModel[] | HttpErrorResponseModel> => {
  const controllerName = `Udc/GetByEntryKey/ResolutionResponse`;
  const endpoint: string = environment.api.generic.replace(':controller', controllerName);

  return EffectUtility.getToModel<ResponseResolutionModel[]>(ResponseResolutionModel, endpoint);
};
