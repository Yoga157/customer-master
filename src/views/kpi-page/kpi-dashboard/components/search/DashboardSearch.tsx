...
import DashboardAdvancedSearch from './DashboardAdvancedSearch';
...
const DashboardSearch: React.FC = () => {
  ...
  const onShowAdvancedSearch = useCallback((): void => {
    dispatch(SidebarContainerActions.OPEN(<DashboardAdvancedSearch setSearchText={setSearchText} setBtnCancel={setBtnCancel} />));
  }, [dispatch]);
  ...
  const onSearch = () => {
    if (btnCancel || searchText.length === 0) {
      ...
      if (searchText.length > 1) {
        dispatch(KpiDataActions.requestKpiDashboardDeptSearch(searchText, currentUser.userName, 1, 15));
        dispatch(KpiDataActions.requestKpiDashboardSearch(searchText, currentUser.userName, 1, 15));
        dispatch(KpiDataActions.setActivePageDept(1));
        dispatch(KpiDataActions.setActivePage(1));
  ...
  const isRequesting: boolean = useSelector((state: IStore) => selectRequesting(state, [KpiDataActions.REQUEST_KPI_DASHBOARD_DATA_SEARCH]));
  return (
