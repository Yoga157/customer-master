...
export const ReportViewer = () => {
  ...
  const GetLocalUserByKey = (keyParam) => {
    ...
    if (jsonString !== null) {
      ...
      for (var key of Object.keys(result)) {
        if (key === keyParam) {
          if(key === 'dashboardSecurity'){
            result[key] = [dashboardSecurityRole, ...result[key], "Project Dashboard"];
            result[key] = result[key].filter(item=>item!=='Finance');
            result[key] = result[key].filter(item=>item!=='Accounting');
  ...
  const getItemDashboard = (key) => {
    ...
    // cardMenuTab(
    //   GetLocalUserByKey('dashboardSecurity').findIndex(item=>item.trim()===key)
    // );
  ...
  const dashboardSecurityRole = 
  GetLocalUserByKey('role') === 'Sales' || GetLocalUserByKey('role') === 'SuperAdmin' ? 
    "Sales" : 
    ...
    window.jQuery('#reportViewer1').telerik_ReportViewer({
      ...
      reportSource: {
        ...
        report:
          GetLocalUserByKey('dashboardSecurity') !== undefined && GetLocalUserByKey('dashboardSecurity')?.length >= 1
            ? dashboardPath?.trim() === 'Sales'
              ...
              : dashboardPath?.trim() === 'Project Dashboard'
              ? 'DashboardPMO.trdp'
        ...
        parameters: {
          ...
          AccName: GetLocalUserByKey('userName'),
          Year: currentYears,
          COCODE: 1,
          StartDate: firstDay,
          EndDate: lastDay,
      ...
      scale: dashboardPath?.trim() === 'Project Dashboard' ? '1' : '0.775',
      scaleMode: dashboardPath?.trim() === 'Project Dashboard' ? 'SPECIFIC' : 'FIT_PAGE_WIDTH',
