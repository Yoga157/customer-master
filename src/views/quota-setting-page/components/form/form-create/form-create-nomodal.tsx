...
import { Form, Grid, Segment, Dropdown, Divider, Confirm, DropdownProps, Header, List } from 'semantic-ui-react';
import {
...
import { selectCustomerOptions, selectCustomerPICOptions, selectEmployeeOptions, selectPresalesOptions } from 'selectors/select-options';
...
import { selectEmployeeSearchOptions, selectSingleSearchCustomerSearch } from 'selectors/select-options/EmployeeSelector';
import { selectFormType, selectFunnelSAOutside, selectFunnelSA } from 'selectors/generated-form/GenerateFormSelector';
...
const FunnelForm: React.FC<IProps> = (props: React.PropsWithChildren<IProps>) => {
  ...
  // );
  console.log('picJOB', picJobTitle);
  ...
  const customerStoreSearch = useSelector((state: IStore) => selectCustomerGenerated(state));
  ...
  const employeeStoreSearch = useSelector((state: IStore) => selectSingleSearchCustomerSearch(state));
  const selectSingleCustomer = useSelector((state: IStore) => state.customer.customer);
  ...
  const resultRenderer = ({ image, price, title, description, flag, id, so, funnelGenID, saNo }) => {
    return (
      <div key={id} className="content">
        <div className="price">{flagSearch === 'SA' ? saNo : flagSearch === 'SO' ? 'SO#' + so : funnelGenID}</div>
  ...
  const onResultSelectCustomer = (data: any) => {
    ...
    dispatch(CustomerPICActions.requestCustomerPIC(+data.result.descriptions, currentUser.employeeID));
    console.log(data);
  };

  const onResultSelectFunnelSA = (data: any) => {
    const { projectName, funnelDate, customerName, customerGenID, funnelGenID, saDate, saNo, salesName, so, salesID } = data.result;
  ...
  const selectedFunnelSA: FunnelSARowModel = useSelector((state: IStore) => selectObjectFunnelSA(state));
  ...
  useEffect(() => {
    if (selectedFunnelSA.projectName != 'undefined') {
      ...
      setPICEmailAddr('');
      setPICJobTitle('');
      console.log('FF', selectedFunnelSA);

      dispatch(CustomerActions.requestCustomerByName(selectedFunnelSA.customerName)).then(() => {
        dispatch(CustomerPICActions.requestCustomerPIC(Number(selectedFunnelSA.customerGenID), currentUser.employeeID));
      });
    }
  }, [selectedFunnelSA]);
  ...
  useEffect(() => {
    if (location.state && location.state.type === 'EDIT') {
      ...
      if (location.state.formType === 'BAST' || location.state.formType === 'Contract') {
  ...
  const GetCustomerPICbyEDIT = () => {
    // console.log
    dispatch(CustomerActions.requestCustomerByName(location.state.customerName));
  };
  ...
  const GetByFormID = (formID: string) => {
    ...
    axios.get(environment.api.generic.replace(':controller', controllerName)).then((res) => {
      ...
      dispatch(CustomerPICActions.requestCustomerPIC(customerID, currentUser.employeeID));
  ...
  const onSubmitHandler = (values: any) => {
    ...
    data.generateForm.formID = location.state && location.state.type === 'ADD' ? '' : editValue.formID ?? '';
    ...
    data.generateForm.formDate = values.formDate === undefined ? new Date() : values.formDate;
  ...
  const bRefreshCustomerPIC: boolean = useSelector((state: IStore) => state.customerPIC.refreshPage);
  ...
  const onFocusFunnelSA = () => {
    // dispatch(ModalFirstLevelActions.OPEN(<FunnelSATable history={props.history} formType={formType} />, ModalSizeEnum.Large));
  };
            ...
            <Grid.Row columns="equal">
              ...
              <Grid.Column className="FullGrid767">
                <Field
                  ...
                  defaultValue={location.state && location.state.type == 'EDIT' ? customerName : customerName}
                />
                ...
                <Grid.Row>
                  <Grid.Column width={16}>
                    <Field
                      ...
                      values={projectName}
                      defaultValue={location.state && location.state.type === 'EDIT' ? projectName : projectName}
                      onChange={(value) => setProjectName(value)}
                      disabled={location.state && location.state.type === 'EDIT' ? false : true}
                    />
                  </Grid.Column>
                </Grid.Row>
                <Divider></Divider>{' '}
