...
import BankGaransiAdminTableRow from './table-row/BankGaransiAdminTableRow';
...
const BankGaransiAdminTable: React.FC<IProps> = (props: React.PropsWithChildren<IProps>) => {
  ...
  const reloads = (columns: any) => {
    ...
    dispatch(BankGaransiActions.requestBankGaransiAdmins(currentUser.userName, activePage, pageSize, 0, columns, direction));
  };
  ...
  return (
    <Table sortable id="export" data-cols-width="10,30,20,40,40,60,10,10,15,20,10">
      ...
      <Table.Body>
        ...
        {tableData.rows.map((model: IBankGaransiAdminTableRow) => (
          <BankGaransiAdminTableRow key={model.bankGuaranteeGenID} rowData={model} />
        ))}
      </Table.Body>
    </Table>
  );
...
export default BankGaransiAdminTable;
