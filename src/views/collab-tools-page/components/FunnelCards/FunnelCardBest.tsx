...
const FunnelCardBest: React.FC<IProps> = (
  ...
  const handleClick = (e, titleProps) => {
    ...
    const typeIdx = `BestFew${index}`;
    const findIdx = activeIndexArr.findIndex((item: any) => item === typeIdx);
    let ele = document.getElementById("BestFewGrid");
    let cards = document.getElementById(`BestFew${index}`);
    if (findIdx > -1) {
  ...
  const [bestFewTicket, setBestFewTicket] = useState([]);
  ...
  function handlePagination(type: string) {
    if (type === "next") {
      ...
      setBestFewTicket(CollabTools.slice(startData - 1, endData));
    } else {
      ...
      setBestFewTicket(CollabTools.slice(startData, endData));
    }
  }
  useEffect(() => {
    setBestFewTicket(CollabTools.slice(0, 20));
  }, [CollabTools]);
  return (
    <div className="col">
      <div className="card" style={{ cursor: "default" }}>
        <span
          className="card-header bg-soft-success d-flex justify-content-between"
          // data-toggle="collapse"
          // href="#BestFew"=
          aria-expanded="false"
          aria-controls="BestFew"
        >
      ...
      <div
        data-simplebar
        className={`tasks-wrapper collapse show ${bestFewTicket.length >= 10 &&
          "need-pagination"}`}
          id="BestFewGrid"
      >
        {bestFewTicket?.map(
          (item, index) =>
            item.funnelStatus === "Best Few" && (
              <Accordion styled className="mt-2 mb-1" key={item.funnelGenID}>
                <AccordionTitle
                  active={
                    activeIndex.find(
                      (activeIdx: any) =>
                        activeIdx === `BestFew${item.funnelGenID}`
                    ) !== undefined
                      ? true
                      : false
                  }
                  ...
                  key={index}
                >
                  <span role="button" key={index} id={`BestFew${item.funnelGenID}`}>
                    ...
                    <Grid className="mt-1 py-0 px-3">
                      <Grid.Row columns={2} className="py-0">
                        <Grid.Column>
                          ...
                          <h1 className="badge bg-soft-BestFew mr-1">
                            {item.salesName}
                          </h1>
                        </Grid.Column>
                      ...
                      <Grid.Row columns={1} className="py-0">
                        <Grid.Column>
                          ...
                          <br />
                          <div className="bg-projectName-best">
