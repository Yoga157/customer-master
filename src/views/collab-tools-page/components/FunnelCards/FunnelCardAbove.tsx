...
const FunnelCardAbove: React.FC<IProps> = (
  ...
  const handleClick = (e, titleProps) => {
    ...
    const typeIdx = `AboveFunnel${index}`;
    let activeIndexArr = [...activeIndex];
    const findIdx = activeIndexArr.findIndex((item: any) => item === typeIdx);
    let ele = document.getElementById("AboveFunnelGrid");
    let cards = document.getElementById(`AboveFunnel${index}`);
    if (findIdx > -1) {
  ...
  const [aboveFunnelTicket, setAboveFunnelTicket] = useState([]);
  ...
  function handlePagination(type: string) {
    if (type === "next") {
      ...
      setAboveFunnelTicket(CollabTools.slice(startData - 1, endData));
    } else {
      ...
      setAboveFunnelTicket(CollabTools.slice(startData, endData));
    }
  }
  useEffect(() => {
    setAboveFunnelTicket(CollabTools.slice(0, 20));
  }, [CollabTools]);
  return (
    <>
      <div className="col">
        <div className="card first" style={{ cursor: "default" }}>
          <span
            className="card-header bg-soft-danger d-flex justify-content-between"
            aria-expanded="false"
        ...
        <div
          data-simplebar
          className={`tasks-wrapper collapse show ${aboveFunnelTicket.length >=
            10 && "need-pagination"}`}
            id="AboveFunnelGrid"
        >
          {aboveFunnelTicket?.map(
            (item, index) =>
              item.funnelStatus === "Above Funnel" && (
                <Accordion
                  styled
                  className={`mt-2 mb-1`}
                ...
                >
                  <AccordionTitle
                    active={
                      activeIndex.find(
                        (activeIdx: any) =>
                          activeIdx === `AboveFunnel${item.funnelGenID}`
                      ) !== undefined
                        ? true
                        : false
                    }
                    ...
                    key={index}
                  >
                    <span role="button" key={index} id={`AboveFunnel${item.funnelGenID}`}>
                      ...
                      <Grid className="mt-1 py-0 px-3">
                        <Grid.Row columns={2} className="py-0">
                          <Grid.Column>
                            ...
                            <h1 className="badge bg-soft-AboveFunnel mr-1">
                              {item.salesName}
                            </h1>
                          </Grid.Column>
                        ...
                        <Grid.Row columns={1} className="py-0">
                          <Grid.Column>
                            ...
                            <br />
                            <div className="bg-projectName-above">
...
export default FunnelCardAbove;
