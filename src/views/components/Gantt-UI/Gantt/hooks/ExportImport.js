      ...
      function get_raw() {
        ...
        if (gantt._scale_helpers) {
          var scales = gantt._get_scales(),
            ...
            autosize_min_width = gantt._get_resize_options().x ? Math.max(gantt.config.autosize_min_width, 0) : gantt.config.$task.offsetWidth,
            height = gantt.config.config.scale_height - 1;
  ...
  (function() {
    ...
    function clear_rec_links(data) {
      ...
      for (i = 0; i < data.links.length; i++) {
        ...
        if (gantt.isTaskExists(link.source) && gantt.isTaskExists(link.target) && tasks[link.source] && tasks[link.target]) {
          links[link.id] = link;
        }
      }
